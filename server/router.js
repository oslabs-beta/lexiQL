...
const { getSQLSchema, formatGraphData } = require('./controllers/SQLController');
...
router.get(
  ...
  (req, res, next) => {
    ...
    if (
      res.locals.SQLSchema &&
      typeof res.locals.SQLSchema === 'object' &&
      !Array.isArray(res.locals.SQLSchema)
    ) {
      return next();
    }
...
router.post('/sql-schema', getSQLSchema, createGQLSchema, formatGraphData, (req, res) => {
  res.status(200).json(res.locals);
});
